...
func TestOrderedMigrationsPreservesExecutionOrder(t *testing.T) {
	got := orderedMigrationVersions(t)
	want := []string{
...
func TestOpenDatabaseRunsSchemaMigrationsAndAddsUsageEventRedisFields(t *testing.T) {
	...
	for _, stage := range []string{"initial migration", "reopen"} {
		t.Run(stage, func(t *testing.T) {
			...
			assertStringSlicesEqual(t, sortedOrderedMigrationVersions(t), versions)
		})
	}
...
func TestRunNormalizesLegacyStorageTimesToProjectTimezone(t *testing.T) {
	...
	for _, version := range orderedMigrationVersions(t) {
...
func TestRunAddsSourceToExistingAuthSessions(t *testing.T) {
	...
	if err := db.Exec("DELETE FROM schema_migrations WHERE version = ?", "20260701_add_auth_session_source").Error; err != nil {
		t.Fatalf("mark target migration pending %s: %v", "20260701_add_auth_session_source", err)
	}
	...
	var count int64
	if err := db.Table("schema_migrations").Where("version = ?", "20260701_add_auth_session_source").Count(&count).Error; err != nil {
...
func TestRunAddsModelPriceMultiplierDefaultToExistingPricing(t *testing.T) {
	...
	if err := db.Exec("DELETE FROM schema_migrations WHERE version = ?", "20260702_model_price_multiplier").Error; err != nil {
		t.Fatalf("mark target migration pending %s: %v", "20260702_model_price_multiplier", err)
	}
	...
	var count int64
	if err := db.Table("schema_migrations").Where("version = ?", "20260702_model_price_multiplier").Count(&count).Error; err != nil {
		t.Fatalf("count model price multiplier migration: %v", err)
	}
	if count != 1 {
		t.Fatalf("expected migration %s to be recorded once, got %d", "20260702_model_price_multiplier", count)
	}
}
...
func sortedOrderedMigrationVersions(t *testing.T) []string {
	versions := orderedMigrationVersions(t)

...
describe('UsagePage toolbar styles', () => {
  ...
  it('keeps the Analysis chart presentation aligned with the redesigned Analysis dashboard', () => {
    ...
    expect(analysisPanelStyles).toMatch(/\.costStackSegment\s*\{[\s\S]*?background:\s*linear-gradient\(90deg, color-mix\(in srgb, var\(--cost-segment-color\) 36%, var\(--bg-secondary\)\), var\(--cost-segment-color\)\);/)

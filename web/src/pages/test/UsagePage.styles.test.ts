...
describe('UsagePage toolbar styles', () => {
  ...
  it('keeps the Analysis chart presentation aligned with the redesigned Analysis dashboard', () => {
    ...
    expect(analysisPanelSource).toContain('removeCompositionTooltip')
    ...
    expect(analysisPanelSource).toContain('createCompositionTooltipHandler')
    ...
    expect(analysisPanelStyles).toMatch(/\.compositionUsageBar\s*\{[\s\S]*?background:\s*linear-gradient\(90deg, #3b82f6, #14b8a6\);/)
    ...
    expect(donutCanvasBoxBlock).toContain('width: min(100%, 300px);')
    expect(donutCanvasBoxBlock).toContain('height: 300px;')
    expect(analysisPanelStyles).toMatch(/\.compositionFloatingTooltip\s*\{[\s\S]*?position:\s*fixed;/)
    expect(analysisPanelStyles).toMatch(/\.compositionFloatingTooltip\s*\{[\s\S]*?pointer-events:\s*none;/)
